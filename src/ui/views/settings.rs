...
pub fn render_settings(frame: &mut Frame, area: Rect, state: &AppState) {
    ...
    let auth_status = if state.is_authenticated { "✓ Authenticated" } else { "✗ Not authenticated" };
    let auth_color = if state.is_authenticated { Color::Green } else { Color::Red };
    ...
    .block(Block::bordered().title("Environment Variables"));
